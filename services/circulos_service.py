...
def transferir_casal_circulo(from_id, to_id, pid, paroquia_id, _encontrista_name_by_id):
    ...
    try:
        cur.execute("""
            SELECT id, ano, integrantes_atual, coord_atual_ele, coord_atual_ela
            FROM circulos
            WHERE id = %s
              AND paroquia_id = %s
        """, (from_id, paroquia_id))
        ...
        if ne and na and circ.get("coord_atual_ele") and circ.get("coord_atual_ela"):
            if (
                ...
                cur.execute("""
                    UPDATE circulos
                    SET integrantes_atual = %s,
                        coord_atual_ele = NULL,
                        coord_atual_ela = NULL
                    WHERE id = %s
                      AND paroquia_id = %s
                """, (_ids_to_str(ids), cid, paroquia_id))
            else:
...
def definir_coord_circulo(cid, pid, paroquia_id, _encontrista_name_by_id):
    ...
    try:
        ...
        cur.execute("""
            UPDATE circulos
            SET coord_atual_ele = %s,
                coord_atual_ela = %s
            WHERE id = %s
              AND paroquia_id = %s
        """, (ne, na, cid, paroquia_id))
        conn.commit()
